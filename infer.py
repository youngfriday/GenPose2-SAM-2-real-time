...
def visualize_pose(data:InferDataset, all_final_pose, all_final_length, visualize_pts=False, visualize_image=False):
    ...
    for index, (obj_pose, obj_length) in enumerate(zip(all_final_pose, all_final_length)):
        # print(f"Visualizing object {index}")  # 调试信息
        if visualize_pts:
...
def data_pre(color_path, depth_path, mask_path, meta_path):
    ...
    # 返回处理后的数据
    data = {
        'color': color,  # RGB 格式的彩色图像 (np.ndarray, 形状为 [H, W, 3])
        'depth': depth,  # 单通道深度图像 (np.ndarray, 形状为 [H, W])
        'mask': mask,    # 单通道掩码图像 (np.ndarray, 形状为 [H, W])
        'meta': meta     # 元数据 (字典格式)
    }
    return data
...
def main():
    ...
    ######################################## PARAMETERS ########################################
    # DATA_PATH = '/home/young/GenPose2/assets/000007'                 # Path to the data
    ...
    # color_images=camera.capture_image('rgb')
    # depth_images=camera.capture_image('depth')
    ...
    while True: 
        ...
        #     # 回到 while True: 
        #     continue
        color_path = f'/home/young/segment-anything-2-real-time/output_img/{index}_color.png'
        depth_path = f'/home/young/segment-anything-2-real-time/output_img/{index}_depth.jpg'
        mask_path = f'/home/young/segment-anything-2-real-time/output_img/{index}_mask.png'
